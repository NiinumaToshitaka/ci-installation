...
<4> コンテナのベースになるDockerイメージを指定する。使用するイメージは https://hub.docker.com/r/jenkins/jenkins[jenkins/jenkins - Docker Image | Docker Hub] を参照。
...
JenkinsのURLを指定する。
ここでは外部に公開しないので http://localhost:8080/ とする。
